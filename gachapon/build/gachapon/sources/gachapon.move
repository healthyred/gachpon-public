...
// --------------- Gas Object ------------------------
public struct GasObject has key, store {
    id: UID,
    player: address,
    game_bytes: vector<vector<u8>>,
}

const GAS_ADDRESS: address = @0x40f7783ef530939137274c2e849da8c7f0194dc585bcf641cd6921cfd9bfb781;

public fun create_and_transfer_gas_obj(
    bytes: vector<u8>,
    ctx: &mut TxContext,
) {
    let mut byte_vec = std::vector::empty<vector<u8>>();
    byte_vec.push_back(bytes);
    transfer::public_transfer(GasObject {
        id: object::new(ctx),
        player: ctx.sender(),
        game_bytes: byte_vec,
    }, GAS_ADDRESS);
